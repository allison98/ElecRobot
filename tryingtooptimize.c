...
#define thresholdVolt 2 //50/1000
...
void recieveData (){
  ...
  while(getDigitalSignal()==0){
	...
	//	printf("wait");
		...
		if((overflow_count*65536.0+TH0*256.0+TL0)*(12.0/SYSCLK)*1000 > 600) {
			TR0=0;
			printf("stop");
			PWMStop();	
			return;
  ...
  for( i=1; i<4; i++){
  	...
  	checkCommands();
  					//does activity depending on the command given 
  //	command[0] = 0;
 //	command[1] = 0;
  //	command[2] = 0;
