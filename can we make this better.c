...
void checkCommands (void){
 //checkerror();
 int first_command[4]={0,0,0,0}; 
 int second_command[4]={0,0,0,0}; 
 
 copyArray(command[], 4, first_command[]); //copy the first command 
// if(arrayEqual(command, 4, stop)) PWMStop();
 if (arrayEqual(command,4,forward)) PWMforward(); 
 ...
 waitms(4*358);  //wait a whole period  to get the next signal 
 copyArray(command, 4, second_command); //copy the first command 
 
 if(!(arrayEqual(second_command, 4, command))  //if the next command after any command 
												//is NOT the STOP command then keep doing the prev command
		copyArray(first_command, 4, command);	//copy the same command back into command array
 else 
		copyArray(stop, 4, command);			//copy STOP into command array  
...
//only works if both the arrays are same sized
//copies array1 into array2
void copyArray (int array_1[], int size, int array_2[]){
	int i; 
	for(i=0; i<size; i++){
		array_2[i]=array_1[i];
	}
...
//when we have ONE command after ANOTHER this doesnt work. We check if 
//  the next command after any command is zero or not. If the next command is not zero, just hardcode
//ot to be zero. In order to stop the interrupts from other sources 
void checkerror (void){

	int first_command[4]={0,0,0,0}; 
	int second_command[4]={0,0,0,0}; 
	if(!(arrayEqual(second_command, 4, stop)))  //if the next command after any command 
												//is NOT the STOP command then keep doing the prev command
		copyArray(first_command, 4, command);	//copy the same command back into command array
	else 
		copyArray(stop, 4, command);			//copy STOP into command array 
